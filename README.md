...
### Separate walking and air tuning
The first five tuning rows are Ground walking speed (16), Ground walking acceleration (100), Aerial movement speed (64), Aerial acceleration (240), and Gravity (60). They support existing value/min/max edits, per-row resets, and master snapshots. Old saves migrate these fields automatically. Gravity previews locally and saved master gravity updates the server. Existing DataStore availability requirements still apply.

Walking never captures an airborne WalkSpeed as its ground baseline. Ordinary walking ramps its own command; metal sliding retains its own acceleration, speed and friction. Roblox Humanoid AutoRotate now handles walking, sliding, coasting and aerial turning; there are no instant root-facing CFrame writes. The airborne pose uses straight side-extended arms with neutral elbows/wrists and palms down. Unselected anchor beams use one soft camera-facing ribbon of identical width/color/opacity, independent of anchor size; only the selected beam is stronger. This removes the old radial-layer brightness variation.
...
Ground slide controls work during powered Q/E movement and coasting. W adds modest speed along travel (25 studs/s^2, separate 200 studs/s boost cap); S brakes to zero (120 studs/s^2) without reversing and counters powered ground acceleration. A/D steer relative to travel (65 degrees/s each), preserving speed before slide friction. Five independent tuning sliders include editable bounds, reset and master save. W suppresses coasting friction while boosting. Poses blend back/upright/left/right with combined inputs and retain anchor hand aiming. Keyboard controls are ignored while editing text or in menus. SlideControl.client.luau verifies integration, momentum caps, migration and UI.
